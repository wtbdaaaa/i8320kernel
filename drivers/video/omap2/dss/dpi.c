...
{
	int r;
        DSSDBG("omapdss_dpi_display_enable-----------------\n");//me add
	...
	if (cpu_is_omap34xx() && !cpu_is_omap3630()) {
		...
		if (r){
                 DSSERR("failed to start device-vdds_dsi_reg1111111111\n");
			goto err0;
	          }
    }
#endif
...
{
	DSSDBG("init_display\n");
	//dssdev->suspend = dpi_display_suspend;//me change
	//dssdev->resume = dpi_display_resume;//me change
        return 0;
}
