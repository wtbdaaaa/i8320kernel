...
void dispc_save_context(void)
{
	...
	if (!dispc.base)
{
DSSDBG("dispc_save_context----!dispc.base=1\n");
		return;
}
...
void dispc_restore_context(void)
{
	if (!dispc.base)
{
DSSDBG("dispc_restore_context----!dispc.base=1\n");
		return;
}
	RR(SYSCONFIG);
	/*RR(IRQENABLE);*/
				...
				enum omap_parallel_interface_mode mode)
{
	...
	if (OMAP_DSS_CHANNEL_LCD2 == channel) {
		...
		printk(KERN_INFO "OMAP DISPCONTROL read (gpout0)%d\n",FLD_GET(l, 15, 15));
		printk(KERN_INFO "OMAP DISPCONTROL read (gpout1)%d\n",FLD_GET(l, 16, 16));
